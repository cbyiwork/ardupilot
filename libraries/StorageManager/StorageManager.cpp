...
const StorageManager::StorageArea StorageManager::layout_default[STORAGE_NUM_AREAS] = {
    ...
    //{ StorageMission, 10028,  5972}, // leave 128 byte gap for expansion

	// add by ycb, for rf id
	{ StorageRfId,    16256,  128},
#endif
};
...
bool StorageAccess::read_block(void *data, uint16_t addr, size_t n) const
{
    ...
    for (uint8_t i=0; i<STORAGE_NUM_AREAS; i++) {
		...
		//hal.uartB->printf("read storage: %d, %d\n", i, STORAGE_NUM_AREAS);
	
        const StorageManager::StorageArea &area = StorageManager::layout[i];

		// type: 0, offset: 0, length: 1280
		//hal.uartE->printf("type: %d, offset: %d, length: %d\n", area.type,area.offset, area.length);
		
        uint16_t length = area.length;
        uint16_t offset = area.offset;

		//hal.uartB->printf("area.type[%d], type[%d]\n", area.type,type);
        if (area.type != type) {
            continue;
        }

		//hal.uartB->printf("addr[%d], length[%d]\n", addr,length);
        if (addr >= length) {
		...
		//hal.uartB->printf("count+addr[%d], length[%d]\n", count+addr,length);
        if (count+addr > length) {
            // the data crosses a boundary between two areas
            count = length - addr;
        }

		//hal.uartB->printf("before hal.storage->read_block(), b = 0x%x, addr = %d, count = %d, n = %d\n", b, addr, count, n);
        hal.storage->read_block(b, addr+offset, count);
        n -= count;

		//hal.uartB->printf("after hal.storage->read_block(), n = %d\n", n);
		//hal.uartB->flush();
...
bool StorageAccess::write_block(uint16_t addr, const void *data, size_t n) const
{
    ...
    for (uint8_t i=0; i<STORAGE_NUM_AREAS; i++) {
		...
		//hal.uartE->printf("%s, write_block, 0x%x, count = %d\n", __func__, addr+offset, count);
