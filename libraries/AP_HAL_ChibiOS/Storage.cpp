...
void Storage::_storage_open(void)
{
    ...
    if (_initialised) {
		//hal.uartE->printf("%s _initialised already\n", __func__);
        return;
    }
    ...
    if (using_fram) {
        if (!fram.read(0, _buffer, CH_STORAGE_SIZE)) {
			//hal.uartE->printf("%s fram.read() error\n", __func__);
            return;
        }
        _save_backup();
        _initialised = true;
    ...
    if (log_fd == -1) {
        //hal.uartE->printf("open failed of " HAL_STORAGE_FILE "\n");
        return;
    }
    int ret = AP::FS().read(log_fd, _buffer, CH_STORAGE_SIZE);
    if (ret < 0) {
        //hal.uartE->printf("read failed for " HAL_STORAGE_FILE "\n");
        AP::FS().close(log_fd);
        log_fd = -1;
        return;
    }
    ...
    if (AP::FS().lseek(log_fd, ret, SEEK_SET) != ret ||
        ...
        //hal.uartE->printf("setup failed for " HAL_STORAGE_FILE "\n");
        AP::FS().close(log_fd);
        log_fd = -1;
        return;        
    }
...
{
	//hal.uartB->printf("%s enter, loc = 0x%x, n = %d\n", __func__, loc, n);
	
    if (loc >= sizeof(_buffer)-(n-1)) {
        return;
    }

	//hal.uartB->printf("%s, _storage_open()\n", __func__);
    _storage_open();

	//hal.uartB->printf("before memcpy, des = 0x%x\n", (uint32_t)dst);
...
void Storage::_flash_load(void)
{
	...
	//hal.uartE->printf("Storage: Using flash pages %u and %u\n", _flash_page, _flash_page+1);
    
    if (!_flash.init()) {
		//hal.uartE->printf("_flash.init() error\n");
		while(1);
