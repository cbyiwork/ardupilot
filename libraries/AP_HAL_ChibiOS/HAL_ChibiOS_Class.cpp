...
static void main_loop()
{
	...
	//hal.uartB->printf("g_callbacks->setup()\n");
    g_callbacks->setup();
	//hal.uartB->printf("g_callbacks->setup() ok\n");
	

#ifdef IOMCU_FW
    stm32_watchdog_init();
#elif !defined(HAL_BOOTLOADER_BUILD)
	//hal.uartB->printf("watchdog_enabled()\n");
    // setup watchdog to reset if main loop stops
    if (AP_BoardConfig::watchdog_enabled()) {
        stm32_watchdog_init();
    }
	//hal.uartB->printf("watchdog_enabled() OK\n");

#ifndef HAL_NO_LOGGING
	//hal.uartB->printf("was_watchdog_reset: %d\n", hal.util->was_watchdog_reset());
	...
	//hal.uartB->printf("watchdog_pat()\n");
    schedulerInstance.watchdog_pat();

	//hal.uartB->printf("hal.scheduler->system_initialized()\n");
