...
bool AP_Param::setup(void)
{
	...
	//hal.uartB->printf("hdr.magic[0]: %d, hdr.magic[1]: %d, hdr.revision: %d\n", hdr.magic[0], hdr.magic[1], hdr.revision);
	
	//hal.uartB->printf("k_EEPROM_magic0: %d, k_EEPROM_magic1: %d, k_EEPROM_revision: %d\n", k_EEPROM_magic0, k_EEPROM_magic1, k_EEPROM_revision);
    ...
    if (hdr.magic[0] != k_EEPROM_magic0 ||
		...
		//hal.uartB->printf("bad header in setup - erasing\n");
...
{
	//hal.uartB->printf("%s enter, type: %d, ptr %x, value: %f\n", __func__,
	//							type, (unsigned int)ptr, value);
