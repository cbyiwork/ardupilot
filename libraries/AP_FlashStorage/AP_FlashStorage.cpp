...
bool AP_FlashStorage::init(void)
{
    ...
    for (uint8_t i=0; i<2; i++) {
		...
		//hal.uartE->printf("head[%x], sig[%x]\n",header[i].signature, signature);
        ...
        if (state != SECTOR_STATE_AVAILABLE &&
			...
			//hal.uartE->printf("state is %x\n", state);
            bad_header = true;
        }

        // initialise if bad header
        if (bad_header) {
			//hal.uartE->printf("bad head()\n");
            return erase_all();
            //return true;
        }
    }
    ...
    for (uint8_t i=0; i<2; i++) {
        ...
        if (states[sector] == SECTOR_STATE_IN_USE ||
            ...
            if (!load_sector(sector)) {
				//hal.uartE->printf("load_sector() error\n");
                return erase_all();
            }
        }
    }
    ...
    if (states[first_sector] == SECTOR_STATE_FULL) {
        ...
        if (!write_all()) {
			//hal.uartE->printf("write_all() error\n");
            return erase_all();
        }
    }
    ...
    for (uint8_t i=0; i<2; i++) {
        if (states[i] == SECTOR_STATE_FULL) {
            if (!erase_sector(i)) {
				//hal.uartE->printf("werase_sector(%d) error\n", i);
                return false;
            }
        }
    }
...
bool AP_FlashStorage::switch_full_sector(void)
{
	...
	//hal.uartE->printf("%s running switch_full_sector()\n", __func__);
...
{
    if (write_error) {
		//hal.uartE->printf("%s write_error\n", __func__);
        return false;
    }
    //debug("write at %u for %u write_offset=%u\n", offset, length, write_offset);
	//hal.uartE->printf("write at %u for %u write_offset=%u\n", offset, length, write_offset);
...
bool AP_FlashStorage::erase_all(void)
{
    ...
    if (!erase_sector(0)) {
		hal.uartE->printf("%s() erase_sector(0) false\n", __FUNCTION__);
        return false;
    }*/
...
bool AP_FlashStorage::write_all()
{
	...
	//hal.uartE->printf("write_all to sector %u at %u with reserved_space=%u\n",
           //current_sector, write_offset, reserved_space);	   
...
bool AP_FlashStorage::switch_sectors(void)
{
    if (reserved_space != 0) {
        ...
        //hal.uartE->printf("both sectors are full\n");
        return false;
    }
    ...
    //hal.uartE->printf("switching to sector %u\n", new_sector);
    
    // check sector is available
    if (!flash_read(new_sector, 0, (uint8_t *)&header, sizeof(header))) {
		//hal.uartE->printf("check sector is available false\n");
        return false;
    }
    if (header.signature != signature) {
		//hal.uartE->printf("header.signature(0x%x) != signature(0x%x)\n", header.signature, signature);
        write_error = true;
        return false;
    }
    if (SECTOR_STATE_AVAILABLE != (enum SectorState)header.state) {
        ...
        //hal.uartE->printf("both sectors full\n");
        return false;
    }
    ...
    if (!flash_write(current_sector, 0, (const uint8_t *)&header, sizeof(header))) {
		//hal.uartE->printf("flash_write error\n");
        return false;
    }
