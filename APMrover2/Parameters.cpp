...
void Rover::load_parameters(void)
{
    ...
    if (!AP_Param::check_var_info()) {
		...
		//hal.uartB->printf("Bad var table\n");
    ...
    if (!g.format_version.load() ||
		...
		//hal.uartB->printf("Firmware change: erasing EEPROM...\n\n");
	...
	//hal.uartB->printf("AP_Param::load_all()\n");
	...
	//hal.uartB->printf("is_balancebot()\n");

    if (is_balancebot()) {
        g2.crash_angle.set_default(30);
    }

	//hal.uartB->printf("load old_rc_keys\n");
	...
	//hal.uartB->printf("load_all took %uus\n", unsigned(micros() - before));
	...
	//hal.uartB->printf("AP_Param::find_old_parameter\n");
	...
	//hal.uartB->printf("load ff_and_filt_conversion_info\n");
	...
	//hal.uartB->printf("AP_Param::convert_old_parameters()\n");
