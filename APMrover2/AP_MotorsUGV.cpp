...
void AP_MotorsUGV::set_throttle(float throttle)
{
    ...
    if (!hal.util->get_soft_armed()) {
		//hal.uartE->printf("soft not armed\n");
        return;
    }

	//hal.uartE->printf("_throttle[%d]\n",_throttle);
...
void AP_MotorsUGV::output(bool armed, float ground_speed, float dt)
{
    ...
    if (!hal.util->get_soft_armed()) {
		...
		//hal.uartE->printf("%s not soft armed\n", __func__);
        armed = false;
        _throttle = 0.0f;
    }
	...
	//hal.uartE->printf("%s set output _steering[%d], _throttle[%f]\n", __func__, _steering, _throttle);
    ...
    if (_pwm_type == PWM_TYPE_BRUSHED_WITH_RELAY) {
        ...
        switch (function) {
            ...
            case SRV_Channel::k_motor1:
                _relayEvents.do_set_relay(0, relay_high);     // relay 继电器
                break;
